...
export default function DoctorDashboard() {
  ...
  const animatedPending = useCountUp(pendingPatients.length);
  const animatedTotal = useCountUp(queue.length);
  ...
  return (
    <div className="min-h-screen relative">
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        {/* Date selector */}
        <div className="glass-card inline-flex items-center gap-3 mb-6 px-4 py-2.5 rounded-xl border border-white/50 animate-fade-up">
          <label className="text-sm font-medium text-gray-600">Viewing queue for:</label>
          <input
            type="date"
            value={selectedDate}
            onChange={(e) => setSelectedDate(e.target.value)}
            className="border border-gray-200 rounded-lg px-3 py-1.5 text-sm text-gray-700 bg-white/80 focus:outline-none focus:ring-2 focus:ring-brand-blue"
          />
        ...
        <div
          className="bg-brand-blue rounded-2xl shadow-xl overflow-hidden mb-8 text-white p-8 text-center relative animate-fade-up"
        >
           ...
           <h1 className="text-5xl font-bold relative z-10 flex items-center justify-center gap-4">
             {queueLoading && queue.length === 0 ? (
               ...
               <>Total Patients in Queue: {animatedPending}</>
             )}
           </h1>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
          <div
            className="glass-card rounded-xl p-6 shadow-sm border border-white/50 flex items-center gap-4 animate-fade-up"
          ...
          >
            ...
            <div>
              ...
              <p className="text-2xl font-bold text-gray-900">{animatedTotal}</p>
            </div>
          </div>
        ...
        {inProgress && (
          <div className="bg-gradient-to-r from-brand-blue to-emerald-700 rounded-2xl shadow-lg p-6 mb-8 flex flex-wrap justify-between items-center gap-4 text-white animate-fade-up">
            <div>
        ...
        {!inProgress && pendingPatients.length > 0 && (
           <div className="glass-card rounded-2xl shadow-md border border-white/50 p-8 mb-8 text-center animate-fade-up">
        ...
        {/* Queue Table */}
        <div className="glass-card rounded-xl shadow-sm border border-white/50 overflow-hidden animate-fade-up">
          <div className="px-6 py-4 border-b border-white/50 bg-white/30">
            <h3 className="text-lg font-bold text-gray-900">Today's Queue</h3>
          </div>
          <div className="overflow-x-auto">
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="bg-white/30 text-gray-500 text-sm uppercase tracking-wider">
              ...
              <tbody className="divide-y divide-white/40">
                {queue.map((apt) => (
                  <tr key={apt.id} className={`hover:bg-white/40 transition-colors ${apt.status === 'COMPLETED' ? 'opacity-50' : ''}`}>
